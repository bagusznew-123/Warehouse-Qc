...
if (isset($_POST['bsimpan'])) {
    ...
    if (isset($_GET['hal']) && $_GET['hal'] == "edit") {
        // Update
        $ubah = mysqli_query($koneksi, "UPDATE qc_dasarata SET
        ...
        if ($ubah) {
            echo "<script>alert('Data berhasil diubah!');document.location='Dasarata.php';</script>";
        }
    } else {
        // Insert
        $simpan = mysqli_query($koneksi, "INSERT INTO qc_dasarata 
        ...
        if ($simpan) {
            ...
            if ($result === FALSE) {
                echo "<script>alert('Data masuk DB, tapi gagal sync ke Spreadsheet');document.location='Dasarata.php';</script>";
            } else {
                echo "<script>alert('Input sukses. Cek data');document.location='Dasarata.php';</script>";
            }
        }
    }
}
...
// === Edit ===
if (isset($_GET['hal']) && $_GET['hal'] == "edit") {
    $tampil = mysqli_query($koneksi, "SELECT * FROM qc_dasarata WHERE id_barang='$_GET[id]' ");
...
if (isset($_GET['hal']) && $_GET['hal'] == "hapus") {
    $hapus = mysqli_query($koneksi, "DELETE FROM qc_dasarata WHERE id_barang='$_GET[id]' ");
    if ($hapus) {
        echo "<script>alert('Data berhasil dihapus!');document.location='Dasarata.php';</script>";
    }
}
?>
      ...
      <div class="card">
        ...
        <div class="card-body table-responsive">
          <table class="table table-bordered table-striped table-sm">
            ...
            <?php
            $no = 1;
            $tampil = mysqli_query($koneksi, "SELECT * FROM qc_dasarata ORDER BY id_barang DESC");
            ...
            <tr>
              ...
              <td class="text-center">
                <a href="Dasarata.php?hal=edit&id=<?= $data['id_barang'] ?>" class="btn btn-warning btn-sm">Edit</a>
                <a href="Dasarata.php?hal=hapus&id=<?= $data['id_barang'] ?>" 
...
function resetForm() {
  <?php if ($mode_edit): ?>
    window.location.href = "Dasarata.php";
  <?php else: ?>
